...
📁 volume1 (Synology Storage)
  └── 📁 Slack (실제 백업본 전용 상위 폴더)
        ...
        │     ├── 📄 last_successful_ts.txt  # 채널별 백업 상태 기록 파일 (JSON 포맷)
...
- 이 파일은 각 슬랙 채널별로 백업에 성공한 마지막 기준 시점(타임스탬프)을 JSON 구조로 기록하여 정밀한 증분 백업을 가능하게 합니다.
- 가독성 제공: 내부적으로 관리자나 사용자가 직접 확인할 수 있도록 사람이 읽을 수 있는 일시(ts_readable)와 채널 이름(channel_name)이 함께 기록됩니다.
- 최초 구동 시 동작: 만약 해당 파일이 없거나 비어있는 상태에서 스크립트를 처음 구동하면, 제한 없이 과거 모든 파일(시점 0)을 기점으로 파일 수집을 시작합니다. 첫 실행이 완료되면 파일이 자동으로 생성 및 갱신됩니다.

**상태 파일 내부 포맷 예시: **
```json
{
  "C0123456789": {
    "channel_name": "daily-meeting",
    "ts": 1718721600.0,
    "ts_readable": "2026-06-18 23:40:00"
  }
}
```